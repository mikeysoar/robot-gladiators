...
var fightOrSkip = function(){
    ...
    // Validat prompt answer
    if (promptFight === "" || promptFight === null) {
        window.alert("Tou need to provide a valid answer! Please try again.");
        //use return to call it again and stop the reast of this function from running
        return fightOrSkip();
    }

    //convert propmtFight to all lowercase so we can check with les options
    ...
    if (promptFight === "skip") {
        ...
        if (confirmSkip) {
            ...
            //return true idfplayer wants to leave
            return true;
        }
    }    
    return false;
...
var fight = function (enemy) {
    ...
    while (playerInfo.health > 0 && enemy.health > 0) {
        if (isPlayerTurn) {
            //ask player if they'd like to fight or skip usijng fightSkip function
            if (fightOrSkip()) {
...
var startGame = function () {
    ...
    for (var i = 0; i < enemyInfo.length; i++) {
        ...
        if (playerInfo.health > 0) {
            ...
            // pass the pickedenemy.name variabl's value into the fight function, where it will assu,me the value of the enemy.name parameter
...
var endGame = function () {
    ...
    //if player has more money thatn the high score, player has a new high dcore!
    if (playerInfo.money > highScore) {
        localStorage.setItem("highscore", playerInfo.money);
        localStorage.setItem("name", playerInfo.name);

        alert(playerInfo.name + " now has the hgh score of " + playerInfo.money + "!");
    }
    else {
...
var shop = function () {
    ...
    // use switch to carroy out option
